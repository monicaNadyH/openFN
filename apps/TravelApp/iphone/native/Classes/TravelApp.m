...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions 
{
    ...
    //added ios8 permissions for notification and sound
...
-(BOOL)application:(UIApplication *)application openURL:(NSURL *)url sourceApplication:(NSString *)sourceApplication annotation:(id)annotation{
    if([[url host] isEqualToString:@"opentravelapp"]){
        if([[url path] isEqualToString:@"/search"]){
            NSMutableDictionary *data = [[NSMutableDictionary alloc] init];
            [data setValue:@"Which Caribbean island is the best destination for a winter vacation with children" forKey:@"question"];
            [[WL sharedInstance] sendActionToJS:@"search" withData:data];
        }else{
